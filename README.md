_<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00F7FF,100:007BFF&height=200&section=header&text=Andy%20Orozco&fontSize=40&fontColor=ffffff&animation=fadeIn&fontAlignY=35" />

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=1000&color=00F7FF&center=true&vCenter=true&width=700&lines=Backend+Developer;Full+Stack+Developer;REST+API+Builder;Laravel+%7C+Node+%7C+Spring;Clean+Code+%26+Scalable+Systems" />

</div>

---

## 🚀 About Me

Hi! I'm Andy 👋 — a Systems Engineering student who enjoys building complete web applications, both on the backend and the frontend.

I like working with REST APIs using PHP (Laravel) and also creating interfaces with technologies like Vue and React, focusing on clean, organized, and scalable solutions.

I've worked in team projects using Scrum, taking on roles like Developer, Scrum Master, and QA, which helped me grow both technically and in teamwork.

I enjoy understanding how everything connects — from the database and backend logic to the user experience — and continuously improving my skills along the way.

---

## 🛠️ Tech Stack

### ⚙️ Backend & Core
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="30" /> Java
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="30" /> Python
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" height="30" /> PHP
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg" height="30" /> Laravel
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" height="30" /> Spring Boot
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="30" /> Node.js
</p>

### 🎨 Frontend
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg" height="30" /> Vue.js
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="30" /> React
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="30" /> HTML5
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="30" /> CSS3
</p>

### 🗄️ Databases
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="30" /> MySQL
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain.svg" height="30" /> SQL Server
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/oracle/oracle-original.svg" height="30" /> Oracle
</p>

### 🧰 Tools & DevOps
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="30" /> Docker
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="30" /> GitHub
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="30" /> Git
</p>

`Postman` `Swagger` `GitHub Actions` `CI/CD` `Scrum` `Kanban` `Railway`

---

## 🧠 What I Do

- Design and build **REST APIs** with clean and scalable architecture  
- Develop backend systems using **Laravel, Node.js, and Spring Boot**  
- Integrate and manage **relational databases**  
- Apply **Clean Architecture** and best practices  
- Work in **agile teams (Scrum)**  
- Perform **testing and QA validation**  

---

## 📫 Connect with me

<p>
  <a href="https://mail.google.com/mail/?view=cm&to=andyorozcocastro98@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/andy-orozco">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/andyorozcoo">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

## ⚡ Personal Note

> Always willing to learn new technologies, improve my skills, and grow as a developer 🚀

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=00F7FF&height=100&section=footer" />
</div>
